...
底层使用 `flash` 库（POSIX shell 解析器）将命令解析为 AST，walk 节点提取命令和运算符，不自写解析器。
...
**访问裁决（AccessVerdict）**：
沙盒对 URI 访问请求的判定结果，包含 `Allowed`、`ReadOnly`、`Denied` 三种状态。由 `playpen_sandbox::Sandbox::access()` 或 `diagnostic::check_path_access()` / `check_domain_access()` 返回。
_避免使用_：权限、许可
