...
use crate::config::policy::ShellPolicy;
...
#[test]
fn pipe_in_quotes_not_detected() {
    let p = parse_shell("echo 'hello|world'");
    assert_eq!(p.commands.len(), 1);
    assert!(!p.has_pipe);
}

#[test]
fn semicolon_in_double_quotes_not_detected() {
    let p = parse_shell("echo \"hello;world\"");
    assert_eq!(p.commands.len(), 1);
    assert!(!p.has_multiple);
}
